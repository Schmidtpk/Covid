...
# drop recent past --------------------------------------------------------
data <- data %>% filter(date< today()-as.difftime(7,units = "days"))
...
# model -------------------------------------------------------------------
jags.m <- jags.model( file = "./jags models/estimateR/stan estimateR predict.R",
                      data=dat,
                      inits=init_fun,
                      n.chains=8,
                      n.adapt=1000)
...
# save(samps,file = "./jags models/save/estimateR_Ger3")
# save(samps,file = "./jags models/save/estimateR_international2")
# load(file = "./jags models/save/estimateR_international2")
# load(file = "./jags models/save/estimateR_Ger2")
...
df <- R %>% left_join(data,by=c("date"="date","country"="country"))


treatments <- colnames(df)[grepl("tt_", colnames(df)) & !grepl("NA", colnames(df))]
treatments_formula <- paste(treatments,collapse = "+")


df <- df %>% group_by(country) %>%
  filter(
    date < max(date)-as.difftime(25,units = "days"),
    date > min(date)+as.difftime(20,units = "days")
    )

summary(
  lm(paste0("R~-1+factorize(cloud)+factorize(tMax)+factorize(humidity)+
            as.factor(t.x)+as.factor(country)+",treatments_formula),
   data = df))
